...
title('Original image - grey scale')
%% Q2_2 - Mean vs Median Filter
% 1
...
img2_2_1 = mean_filter(greyScale_normalize_img,3);
figure;
imshow(img2_2_1,[0 1]);
title('Q2.2 : Mean filter 3x3')

% 2 
img2_2_2 = median_filter(greyScale_normalize_img,3);
figure;
imshow(img2_2_2,[0 1]);
title('Q2.2 : Median filter 3x3')

%% Q2_3 - Gaussian Filter
img2_3_1 = dip_gaussian_filter(greyScale_normalize_img,3);
figure;
imshow(img2_3_1,[]);
title('Q2.3 : Gaussian filter 3x3 with variance = 0.3')

img2_3_3 = greyScale_normalize_img - img2_3_1;
figure;
imshow(img2_3_3,[]);
title('Q2.3 : Subtract image')

%% Q2_4 - Anisotropic Diusion Filter
...
img  = anisodiff2D(greyScale_normalize_img , 2  ,1/7 ,10 ,1);
img2 = anisodiff2D(greyScale_normalize_img , 8  ,1/7 ,10 ,1);
img3 = anisodiff2D(greyScale_normalize_img , 16 ,1/7 ,10 ,1);
img4 = anisodiff2D(greyScale_normalize_img , 2  ,1/7 ,10 ,2);
img5 = anisodiff2D(greyScale_normalize_img , 8  ,1/7 ,10 ,2);
img6 = anisodiff2D(greyScale_normalize_img , 16 ,1/7 ,10 ,2);

imagesArrayQ1_3 = [img, img2, img3;img4, img5, img6];
montage(imagesArrayQ1_3 , 'DisplayRange', [0 1]);
title ('Q2_4 (k=10) : t = 2                                                                                                              t = 8                                                                                                       t = 16');
figure;
img = anisodiff2D(greyScale_normalize_img , 2 ,1/7 , 30 ,1);
img2 = anisodiff2D(greyScale_normalize_img , 8 ,1/7 , 30 ,1);
img3 = anisodiff2D(greyScale_normalize_img , 16 ,1/7 , 30 ,1);
img4 = anisodiff2D(greyScale_normalize_img , 2 ,1/7 , 30 ,2);
img5 = anisodiff2D(greyScale_normalize_img , 8 ,1/7 , 30 ,2);
img6 = anisodiff2D(greyScale_normalize_img , 16 ,1/7 , 30 ,2);

imagesArrayQ1_3 = [img, img2, img3;img4, img5, img6];
montage(imagesArrayQ1_3 , 'DisplayRange', [0 1]);
title ('Q2_4 (k=30) : t = 2                                                                                                              t = 8                                                                                                       t = 16');

%% Q2_5 - Noise Filtering
...
title('Q2.5 : Salt & Pepper : Original                 mean filter                                       median filter                                       gaussian filter                                       anisotropic  filter')
...
title('Q2.5 : Gaussian : Original                      mean filter                                       median filter                                       gaussian filter                                       anisotropic  filter')
...
title('Q2.5 : Speckle : Original                       mean filter                                       median filter                                       gaussian filter                                       anisotropic  filter')

%% Q2_6 - Larger Kernels
% Repeat sections 2.2, 2.3 and 2.5. With filter size of 9x9. Explain the dierence.
%   Q2_2
img2_6_2_1 = mean_filter(greyScale_normalize_img,9);
figure;
imshow(img2_6_2_1,[0 1]);
title('Q2.6 : Mean filter 9x9')

img2_6_2_2 = median_filter(greyScale_normalize_img,9);
figure;
imshow(img2_6_2_2,[0 1]);
title('Q2.6 : Median filter 9x9')

%   Q2_3
img2_6_3_1 = dip_gaussian_filter(greyScale_normalize_img,9);
figure;
imshow(img2_6_3_1,[]);
title('Q2.6 : Gaussian filter 9x9')

img2_6_3_3 = greyScale_normalize_img - img2_6_3_1;
figure;
imshow(img2_6_3_3,[]);
title('Q2.6 : Subtract image')
...
imagesArray_salt_pepper = [img_salt_pepper, img_salt_pepper1,img_salt_pepper2, img_salt_pepper3, img_salt_pepper4];
montage(imagesArray_salt_pepper , 'DisplayRange', [0 1]);
title('Q2.6 : Salt & Pepper : Original                 mean filter                                       median filter                                       gaussian filter                                       anisotropic  filter')

figure('Name','Gaussian');
imagesArray_gaussian = [img_gaussian, img_gaussian1, img_gaussian2 , img_gaussian3, img_gaussian4];
montage(imagesArray_gaussian , 'DisplayRange', [0 1]);
title('Q2.6 : Gaussian : Original                      mean filter                                       median filter                                       gaussian filter                                       anisotropic  filter')

figure('Name','Speckle');
imagesArray_speckle = [img_speckle, img_speckle1, img_speckle2, img_speckle3, img_speckle4];
montage(imagesArray_speckle , 'DisplayRange', [0 1]);
title('Q2.6 : Speckle : Original                       mean filter                                       median filter                                       gaussian filter                                       anisotropic  filter')
...
function Filtered_image = dip_gaussian_filter(img,Thesize)
    ...
    for n = 1:rows
        for m = 1:columns
            ...
            for i = -var:var
                for j = -var:var
                    sum = sum + paddedImage(n+i+var,m+j+var)*G(i+var+1,j+var+1);
                end 
            end
            Filtered_image(n,m) = sum;
        end
    end
